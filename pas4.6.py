...
combined_df.iloc[:, 3:] = combined_df.iloc[:, 3:].replace(missing_value, np.nan)

# Calcular totales y medias anuales para cada estación (por fila)
combined_df["Annual_Total"] = combined_df.iloc[:, 3:].sum(axis=1, skipna=True)  # Sumar sin considerar NaN
combined_df["Annual_Mean"] = combined_df.iloc[:, 3:].mean(axis=1, skipna=True)  # Promediar sin considerar NaN

# Agrupar por año y calcular los totales y medias anuales de todas las estaciones
annual_stats = combined_df.groupby("Year")[["Annual_Total", "Annual_Mean"]].agg(['sum', 'mean'])

# Calcular la media anual combinada de todas las estaciones para cada año
annual_combined_mean = combined_df.groupby("Year")[["Annual_Mean"]].mean()
...
for year in annual_stats.index:
    ...
    print(f"Promedio mensual de todas las estaciones en {year}: {annual_combined_mean.loc[year, 'Annual_Mean']:.2f}")
